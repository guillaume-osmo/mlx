...
class TestTurboQuantFast(mlx_tests.MLXTestCase):
    @unittest.skipIf(not mx.is_available(mx.gpu), "No GPU available")
    def test_qk_packed_scores_matches_reference(self):
        bits = 3
        n_queries = 11
        n_keys = 13
        dim = 32
        ...
        scores_fused = qk_packed_scores(q_rot, k_packed, k_norms, centroids, bits)

        k_deq = dequant_reference(k_idx, k_norms, centroids)
        scores_ref = q_rot @ k_deq.T

        self.assertTrue(mx.allclose(scores_fused, scores_ref, rtol=1e-4, atol=1e-4))
