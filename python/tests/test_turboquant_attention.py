...
Verifies:
  ...
  6. Multiple head dimensions (D=64, D=128, D=256)
"""
...
class TestTurboQuantAttention(mlx_tests.MLXTestCase):
    ...
    def test_output_shapes_d256(self):
        """Works with D=256 head dimension."""
        inputs = self._make_inputs(B=1, H_q=8, H_kv=2, N=64, D=256)
        acc, m, l = mx.fast.turboquant_attention(**inputs)
        mx.eval(acc, m, l)

        self.assertEqual(acc.shape, (1, 8, 1, 256))
        self.assertEqual(m.shape, (1, 8, 1))
        self.assertEqual(l.shape, (1, 8, 1))
    ...
    def test_4bit_d256_gqa(self):
        """4-bit with D=256 and GQA."""
        inputs = self._make_inputs_4bit(B=1, H_q=16, H_kv=2, N=256, D=256)
        acc, m, l = mx.fast.turboquant_attention(**inputs)
        mx.eval(acc, m, l)

        self.assertEqual(acc.shape, (1, 16, 1, 256))
        self.assertTrue(mx.all(mx.isfinite(acc)).item())
        self.assertTrue(mx.all(l > 0).item())
    ...
    def test_mixed_2bit_keys_4bit_values_d256(self):
        """2-bit keys with 4-bit values at D=256."""
        inputs = self._make_inputs_4bit(
            B=1, H_q=16, H_kv=2, N=2048, D=256, mse_bits=2, v_bits=4
        )
        acc, m, l = mx.fast.turboquant_attention(**inputs)
        mx.eval(acc, m, l)
